...
contract ERC7092 is IERC7092, BondStorage {
    ...
    function bondManager() public view returns(address){
        return _bondManager;
    }

    function issue(IssueData[] memory _issueData,Bond memory _bond) 
    external onlyBondManager {_issue(_issueData, _bond);}

    function redeem() external onlyBondManager {
        _redeem(_listOfInvestors);
    }

    function _issue(IssueData[] memory _issueData, Bond memory _bondInfo)
    internal virtual {
        uint256 volume;
